...
func (k *kafkaService) KafkaInit() {
	...
	k.ChatWriter = &kafka.Writer{
		...
		Balancer:               &kafka.Hash{},		// 使用 Hash 负载均衡策略，确保相同 Key 的消息发送到同一分区（如同一用户的聊天消息集中处理）。
		WriteTimeout:           kafkaConfig.Timeout * time.Second,
		RequiredAcks:           kafka.RequireNone,
		AllowAutoTopicCreation: false,
	}
	k.ChatReader = kafka.NewReader(kafka.ReaderConfig{
		Brokers:        []string{kafkaConfig.HostPort},	// 指定 Kafka broker 列表（支持多个节点，当前仅读取单个配置地址）。
		Topic:          kafkaConfig.ChatTopic,			// 绑定与写入器一致的聊天 Topic，确保读写同一份消息
		CommitInterval: kafkaConfig.Timeout * time.Second,
		GroupID:        "chat",							// 指定消费组 ID 为 “chat”，同一消费组内的消费者会分摊 Topic 分区的消息（实现负载均衡消费）。
